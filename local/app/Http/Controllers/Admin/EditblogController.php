...
class EditblogController extends Controller
{
    ...
    protected function blogdata(Request $request)
    {
	   ...
	   $mp3path = base_path('images/mp3/'.$save_audio);
	   ...
	   else
	   {
		   ...
		   if($code=="en")
			{
			  ...
			  DB::update('update post set post_title="'.$pagename.'",post_slug="'.$pther.'",post_desc="'.htmlentities($postdesc).'",post_tags="'.$post_tags.'",post_type="'.$post_type.'",lang_code="'.$code.'",post_media_type="'.$media_type.'",post_image="'.$namef.'",post_audio="'.$mp3name.'",post_video="'.$videourl.'",post_date="'.$post_date.'" where post_id = ?', [$post_id]);
			...
			DB::insert('insert into post (post_title,post_slug,post_desc,post_tags,post_type,token,lang_code, parent,post_media_type,post_image,post_audio,post_video,post_date,post_status) values (?, ?, ?, ? ,?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [$pagenamo,$pther,htmlentities($postdeco),$post_tags,$post_type,$token,$code,$post_id,$media_type,$imgnamer,$mpname,$videourl,$post_date,$post_status]);
				 ...
				 DB::update('update post set post_title="'.$pagename.'",post_slug="'.$pther.'",post_desc="'.htmlentities($postdesc).'",post_tags="'.$post_tags.'",post_type="'.$post_type.'",post_media_type="'.$media_type.'",post_image="'.$namef.'",post_audio="'.$mp3name.'",post_video="'.$videourl.'",post_date="'.$post_date.'" where lang_code="'.$code.'" and parent = ?', [$post_id]);
		...
		/*DB::update('update post set post_title="'.$post_title.'",post_slug="'.$post_slug.'",post_desc="'.$post_desc.'",post_tags="'.$post_tags.'",post_type="'.$post_type.'",post_media_type="'.$media_type.'",post_image="'.$namef.'",post_audio="'.$mp3name.'",post_video="'.$videourl.'",post_date="'.$post_date.'" where post_id = ?', [$post_id]);*/
