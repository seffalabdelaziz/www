...
if(!empty($stripe_token))
{
   ...
   DB::update('update product_orders set payment_token="'.$stripe_token.'" where status="completed" and purchase_token = ?', [$cid]);
   DB::update('update product_checkout set payment_token="'.$stripe_token.'" where payment_status="completed" and purchase_token = ?', [$cid]);
}
...
<head>
   ...
   @include('style')
	 <title><?php echo translate( 25, $lang);?> - <?php echo translate( 469, $lang);?></title>
...
@if(Session::has('success'))
    ...
    <?php if(!empty($stripe_token)){ ?><label class="black"><?php echo translate( 475, $lang);?> : <?php echo $stripe_token;?> </label><?php } ?>
