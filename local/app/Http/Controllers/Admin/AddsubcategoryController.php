...
class AddsubcategoryController extends Controller
{
...

    {
        $language = DB::table('avig_language')
					...
					->orderBy('id','asc')
					->get();
        return view('admin.addsubcategory', ['language' => $language]);
    ...
    {   
        ...
        return view('admin.addsubcategory', ['category' => $category,'language' => $language]);
    ...
    protected function addsubcategorydata(Request $request)
    {
			   ...
			   else
			   {
			       ...
			       $subcategory = DB::table('subcategory')
					...
					->get();
					 ...
					 $subcategory_cnt = DB::table('subcategory')
		           		->where('token', '=', $token)
						->where('subcat_type','=',$subcat_type)
					->where('parent', '=', 0)
					->count();
	   ...
	   else
	   {
		...
		$idd = DB::table('subcategory')-> insertGetId(array(
