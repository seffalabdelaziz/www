...
class EditsubframeworkController extends Controller
{
	 ...
	 $subcategory = DB::select('select * from subcategory where delete_status="" and subid = ?',[$id]);
       ...
       $category = DB::table('category')
		...
		$data = array('subcategory'=>$subcategory, 'category'=>$category, 'language' => $language);
            return view('admin.edit_framework_subcategory')->with($data);
		   ...
		   if($code=="en")
			{
			  ...
			  DB::update('update subcategory set subcat_name="'.$pagename.'",post_slug="'.$pther.'",cat_id="'.$cat_id.'",subimage="'.$savefname.'",lang_code="'.$code.'",status="'.$status.'",subcat_type="'.$subcat_type.'" where subid = ?', [$subid]);
			}
			else
			{
			    $counts = DB::table('subcategory')
			     ...
			     if($counts==0)
				 {
				     ...
				     DB::insert('insert into subcategory (subcat_name, 	post_slug, cat_id, subimage,  lang_code, token, parent, status, subcat_type) values (?, ?, ?, ?, ?, ?, ?, ?, ?)', [$pagenamo,$pther, $cat_id, $savefname,$code,$token,$subid, $status, $subcat_type]);
				 }
				 else
				 {
				   DB::update('update subcategory set subcat_name="'.$pagename.'",post_slug="'.$pther.'",cat_id="'.$cat_id.'",subimage="'.$savefname.'",status="'.$status.'",subcat_type="'.$subcat_type.'" where lang_code="'.$code.'" and parent = ?', [$subid]);
				 }
