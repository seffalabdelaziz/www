...
{
   $tx = $_REQUEST['tx'];
   DB::update('update product_orders set payment_token="'.$tx.'" where status="completed" and purchase_token = ?', [$cid]);
   DB::update('update product_checkout set payment_token="'.$tx.'" where payment_status="completed" and purchase_token = ?', [$cid]);
}
...
@if(Session::has('success'))
    ...
    <?php if(!empty($_REQUEST['tx'])){ ?><label class="black"><?php echo translate( 475, $lang);?> : <?php echo $_REQUEST['tx'];?> </label><?php } ?>
